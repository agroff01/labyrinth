...
namespace CustomUtils
{
    public static class VectorUtils
    {
        ...
        public static void Abs(this Vector2 value)
        {
            value.x = Mathf.Abs(value.x);
            value.y = Mathf.Abs(value.y);
        }

        public static Vector2 AbsCopy(this Vector2 value) => new(Mathf.Abs(value.x), Mathf.Abs(value.y));

        public static void ComponentWiseClamp(this Vector2 value, Vector2 min, Vector2 max)
        {
        ...
        public static float Max(this Vector2 val) => Mathf.Max(val.x, val.y);

        public static float LerpRange(this Vector2 vec, float val) => Mathf.Lerp(vec.x, vec.y, val);

        public static bool CheckAsRange(this Vector2 range, float value)
        {
            return range.x <= value && value <= range.y;
        }

        public static Vector2 WithX(this Vector2 val, float newX) => new(newX, val.y);
        public static Vector2 WithY(this Vector2 val, float newY) => new(val.x, newY);
        ...
        public static void ComponentWiseClamp(this Vector3 value, Vector3 min, Vector3 max)
        {
            value.x = Mathf.Clamp(value.x, min.x, max.x);
            value.y = Mathf.Clamp(value.y, min.y, max.y);
        ...
        public static float Max(Vector3 value) => Mathf.Max(Mathf.Max(value.x, value.y), value.z);

        public static Vector3 AbsCopy(this Vector3 value)
        {
        ...
        public static Vector3 Swizzle(this Vector3 vec, Axis newx, Axis newy, Axis newz) => new(vec.ByAxis(newx), vec.ByAxis(newy), vec.ByAxis(newz));
